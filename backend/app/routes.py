...
api2 = Blueprint('api/v2', __name__)


@api2.route('/menues/<id>/today')
def meal_menu_today(id):
    stmt = db.select(CurrentFood.food_id, CurrentFood.available,Food.name , Food.category, Food.price_student,Food.food_type,  db.func.round(db.func.avg(Rating.points), 2).label('avg'))\
                                   .where(CurrentFood.canteen_id == id)\
                                   .where(CurrentFood.last_available >= date.today())\
                                   .join(Rating, Rating.food_id == CurrentFood.id) \
                                   .join(Food, Food.id == CurrentFood.food_id)\
                                   .group_by(CurrentFood.id, Food.name, Food.category,Food.food_type,  Food.price_student)


    print(stmt)
    currfoods = db.session.execute(stmt)


    return ratings_schema2.dump(currfoods)


@api2.route('/canteens', methods=['GET'])
def get_canteens2():
    canteens = db.session.execute(db.select(Canteen))
    return cateen_schema.dump(canteens.scalars().all())



@api2.route('/food_detail/<food_id>', methods=['GET'])
def get_food_details2(food_id):
    food = db.session.execute(db.select(Food).where(Food.id == food_id)).scalars().first()
    return food_schema.dump(food)
