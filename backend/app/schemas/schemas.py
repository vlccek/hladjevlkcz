...
class CurrentFoodSchema(ma.SQLAlchemyAutoSchema):
    food = ma.Nested(FoodSchema, attribute='food')
...
class Testing(Schema):
    avg = fields.Float(as_string=False)
    food_id = fields.Integer()
    #food = ma.Nested(FoodSchema)
    name = fields.String()
    category = ma.List(ma.Enum(FoodCategoryEnum))
    price_student= fields.Integer()
    food_type = fields.Integer()
    available = fields.Boolean()
    # average = fields.Method("getAverage", required=True)
    #hovnokodjehovno = ma.Nested(FoodRatingStatisticsSchema, attribute='id', only=['average', 'based_on'])
    # cur_food = ma.Nested(CurrentFood, attribute='')
    class Meta:
        model = CurrentFood



ratings_schema2 = Testing(many=True)


class CanteenS(ma.SQLAlchemyAutoSchema):
    class Meta:
        # fields = ("id", "name")
        model = Canteen
        load_instance = True

cateen_schema = CanteenS(many=True, exclude=['address', "opened_first", "opened_first_friday", "opened_second", "opened_second_friday"])
